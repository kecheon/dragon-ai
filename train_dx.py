...
print(f"--- Starting training process for {SYMBOL} ---")

# data_loader 모듈을 사용하여 데이터 로드
data = load_price_data(SYMBOL, TIMEFRAME)

# 데이터가 성공적으로 로드되었는지 확인
if data.empty:
    print(f"{SYMBOL} 데이터를 로드하는 데 실패했거나 데이터가 없습니다. 스크립트를 중단합니다.")
    exit() # 데이터가 없으면 이후 코드를 실행할 수 없으므로 종료
